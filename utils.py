...
class PolyLoss(nn.Module):
    """PolyLoss (ICLR 2022) — strong for medical imaging."""
...
def get_loss_fn(loss_name, class_weights):
    if loss_name == 'ce':
        ...
        return PolyLoss(num_classes=len(class_weights), epsilon=1.0,
...
def train_one_epoch(model, loader, criterion, optimizer, device, epoch,
                    total_epochs=None, use_mixup=True):
    """
    Args:
        total_epochs: if provided, mixup is disabled after cfg.MIXUP_END_RATIO * total_epochs
    """
    model.train()
    running_loss = 0.0
    all_preds, all_labels = [], []

    # ── Disable mixup in the later epochs so the model learns clean boundaries ──
    mixup_active = use_mixup
    if use_mixup and total_epochs is not None:
        mixup_active = epoch <= int(total_epochs * cfg.MIXUP_END_RATIO)

    # ── Proper AMP: scaler + autocast together ─────────────────────────────
    use_amp = (device == "cuda")
    ...
    for images, labels in pbar:
        ...
        if mixup_active and np.random.rand() > 0.5:
...
def generate_submission_live(models_by_fold, test_loader, device, label2name,
    ...
    for images, ids in tqdm(test_loader, desc="Live inference", leave=False):
        ...
        for config_name in config_names:
            ...
            fold_avg = fold_avg / len(fold_models)  # type: ignore
