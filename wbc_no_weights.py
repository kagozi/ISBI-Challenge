...
==================================================
VERSION: NO CLASS WEIGHTS / NO WEIGHTED SAMPLING
- Simpler baseline approach
- Still includes: PolyLoss, AutoGluon, better models, improved augmentation
"""
...
print(f"Phase 2 Eval:     {len(phase2_eval_df):6,} images")
print(f"{'─'*70}")
...
# ============================================================================
# 2. LOSS FUNCTIONS (NO CLASS WEIGHTS)
# ============================================================================
...
class PolyLoss(nn.Module):
    """Poly Loss from winning solution"""
...
def get_loss_fn(loss_name, num_classes):
    """Get loss function (NO class weights)"""
    if loss_name == 'ce':
        return nn.CrossEntropyLoss()
    elif loss_name == 'ce_smooth':
        return LabelSmoothingCrossEntropy(epsilon=0.1)
    elif loss_name == 'poly':
        return PolyLoss(num_classes=num_classes, epsilon=1.0)
    else:
...
class HybridSwin(nn.Module):
    def __init__(self, num_classes, dropout=0.4, pretrained=True):
        ...
        print(f"  HybridSwin: {sum(p.numel() for p in self.parameters())/1e6:.1f}M params")
...
class ConvNeXt(nn.Module):
    def __init__(self, num_classes, dropout=0.4, pretrained=True):
        ...
        print(f"  ConvNeXt: {sum(p.numel() for p in self.parameters())/1e6:.1f}M params")
...
class MaxViT(nn.Module):
    def __init__(self, num_classes, dropout=0.4, pretrained=True):
        ...
        print(f"  MaxViT: {sum(p.numel() for p in self.parameters())/1e6:.1f}M params")
...
# ============================================================================
# 7. FEATURE EXTRACTION FOR AUTOGLUON
# ============================================================================
...
def train_model(config, train_loader, val_loader, num_classes, device, save_dir='models_autogluon'):
    ...
    criterion = get_loss_fn(loss_name, num_classes)  # NO class weights
...
def main():
    ...
    # Create data loaders (NO weighted sampling - simple shuffle)
    ...
    print("⚠️  NOTE: Using simple shuffle (NO weighted sampling)")
    print("⚠️  NOTE: Using NO class weights in loss functions\n")
